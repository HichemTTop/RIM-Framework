...
class HSIBmodel():
    ...
    def SetParameters(self, opinion_set):
        ...
        for i, each in enumerate(self.ListInfectedNodes):
            self.Graph.nodes[each]['Infetime'] = self.Tdet
    ...
    def neighbor(self):
        ...
        for i in range(self.Graph.number_of_nodes()):
            ...
            accp_C.append(self.Graph.nodes[i]['AccpR_C'])
            ...
            for j in n:
                if j not in self.ListInfectedNodes:
                    if j not in neighb:
                        ...
                        PR.append(self.Graph.nodes[j]['page_rank'])

        return neighb, MaxD, deg_cent, beta, betaD, betaDJ, judgement, betweenness_centrality,Accp_R,PR

    def blocking_methods(self, nb_nodes_toBlock, nodes_degree, degree_centrality, Beta, betaD, betaDJ, judgement, betweenness_centrality,Accp_R,PR, method):
        ...
        if method == 'MINJUGBN':  # judgement_Blocking_nodes
            node_to_block = judgement.index(min(judgement))
            judgement.pop(node_to_block)
        ...
        if method == 'PR':
            node_to_block = PR.index(max(PR))
            PR.pop(node_to_block)
        if method == 'ARM':
            node_to_block = Accp_R.index(max(Accp_R))
            Accp_R.pop(node_to_block)
            
        return node_to_block

    def Block_nodes(self, method):
        neighbors, nodes_degree, degree_centrality, Beta, betaD, betaDJ, judgement, betweenness_centrality,Accp_R,PR = self.neighbor()
        ...
        for i in range(nb_nodes_toBlock):
            node_to_block = self.blocking_methods(
                nb_nodes_toBlock-i, nodes_degree, degree_centrality, Beta, betaD, betaDJ, judgement, betweenness_centrality,Accp_R,PR, method)
    ...
    def TCS_methods(self, nb_nodes_toUse, degree, degree_centrality, Beta, betaD, betaDJ, judgement, betweenness_centrality,Accp_R,PR, method):
        if method == 'RTCS':
        ...
        if method == 'ARM':
            node_to_use = Accp_R.index(max(Accp_R))
            Accp_R.pop(node_to_use)
        return node_to_use

    def Truth_campaign_strategy(self, method):
        neighbors, degree, degree_centrality, Beta, betaD, betaDJ, judgement, betweenness_centrality,Accp_R,PR = self.neighbor()
        nb_neighbors = len(neighbors)
        ...
        for i in range(nb_nodes_toUse):
            node_to_use = self.TCS_methods(nb_nodes_toUse-i, degree, degree_centrality,
                                           Beta, betaD, betaDJ, judgement, betweenness_centrality,Accp_R,PR, method)
    ...
    def runModel(self, i=0, typeOfSim=1, Stat=0):
                    ...
                    if (c <= ActualAttraction*0.5):
                        ...
                        for each in new_ones:
                            ...
                            if (self.Graph.nodes[each]['blocked'] == 'false'):
                                if(np.random.rand() <= ProbToAccRumor*0.2):
        ...
        if self.verbose:
            print(f'Simulation number {i} has finished')
